# Introducing Myself 

<html> 
<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=30&duration=3500&pause=1000&color=00F2FE&center=true&vCenter=true&width=700&lines=Hi+there%2C+I'm+Thanishka;+Building+the+Future+with+AI+%26+ML;+Full-Stack+Developer+%7C+Cloud+Enthusiast" alt="Typing Effect" />
</div> 
</html>
 
**B.Tech Student in Artificial Intelligence & Machine Learning**  
Bengaluru, India  

A focused and detail-oriented engineering student specializing in AI/ML, cloud infrastructure, and full-stack development. I enjoy building practical software solutions, automating workflows for efficiency, and consistently refining my problem-solving skills through daily technical challenges.

---

### 🛠️ Technical Focus

* **Languages:** Python, C++, JavaScript
* **Backend & Web Frameworks:** Node.js, Express.js
* **Cloud & Automation:** Google Cloud Platform (GCP), Bash Scripting, Git/GitHub

---

### 💻 Key Projects

#### **Dhwani**
*A real-time translation and visual assistant application designed to bridge communication gaps.*
* Developed a responsive full-stack architecture using Node.js and Express.js.
* Integrated multimodal AI and language APIs to handle real-time audio and visual processing.

#### **NutriSense Pro**
*An AI-powered nutrition advisory platform delivering personalized insights.*
* Built a clean, functional backend infrastructure to securely process user data.
* Handled end-to-end deployment, API routing, and system optimization.

---

### 📊 GitHub Activity

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Thanishka-K&show_icons=true&theme=neutral&hide_border=true&count_private=true" alt="GitHub Stats" height="165" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Thanishka-K&layout=compact&theme=neutral&hide_border=true" alt="Top Languages" height="165" />
</div>

---

### 🤝 Contact & Professional Networks

* **LinkedIn:** [linkedin.com/in/YOUR_LINKEDIN_USERNAME](https://linkedin.com/in/YOUR_LINKEDIN_USERNAME)
* **Email:** [your.email@example.com](mailto:your.email@example.com)

<br />

<div align="center">
  <sub>© 2026 Thanishka K • Built simply with Markdown</sub>
</div
